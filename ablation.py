...
def get_batch():
    ...
    for _ in range(BATCH_SIZE):
        ...
        abc = [i for i in range(22)]
        random.shuffle(abc)
        tinput=[]
        toutput=[]
        for a in range(len(traceinput)):
            abb=[]
            acc=[]
            for b in range(len(traceinput[0])):
                abb.append(traceinput[a][b]+0.01*np.random.randn())
                acc.append(traceoutput[a][b]+0.01*np.random.randn())
            tinput.append(abb)
            toutput.append(acc)
        ti=[]
        to=[]
